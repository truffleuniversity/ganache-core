...
Database.prototype.initialize = function(callback) {
  ...
  getDir(function(err, directory) {
    ...
    if (self.options.db) {
      store = self.options.db;
      levelup(store, levelupOptions, finishInitializing);
      return;
    } else {
      store = encode(cachedown(directory, filedown), levelupOptions);
      if (!self.options.db_path.includes("/tmp/testrpc")) {
        const ganacheDbConfig = join(directory, ".ganache/db.json");
        if (existsSync(join(directory, "!blocks!")) && !existsSync(ganacheDbConfig)) {
          // Backwards compatability for db's created before 2.3.x
          levelup(store, levelupOptions, finishInitializing);
          return;
        }
        try {
          mkdirSync(join(directory, ".ganache/"));
          writeFileSync(ganacheDbConfig);
        } catch (error) {
          console.log(error);
        }
      }
